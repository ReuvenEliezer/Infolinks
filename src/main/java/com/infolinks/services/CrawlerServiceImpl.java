...
//                lines.forEach(line -> logger.info("result from {} : {}", clientInfo.getSiteURL(), line));
                        for (String line : lines) {
                            boolean isValid = line.matches(String.format(".*%1$s.*%2$s.*%2$s.*", DOT_URL_SITE, CSV_SPLITTER));
                            if (isValid) {
                                String[] split = line.split(CSV_SPLITTER);
                                Ads ads1 = new Ads(split[0], split[1], split[2]);
    ...
    @Override
    public Set<String> getAllSite(String siteURL) {
        return lineToUrlSiteMap.get(siteURL);
    }
